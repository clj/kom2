...
def test_invalid_handle(C):
    assert (
        C.SQLBindParameter(C.NULL, 0, 0, 0, 0, 0, 0, C.NULL, 0, C.NULL)
...
def test_invalid_input_output_types(C, stmt_handle, input_output_type):
    assert (
        C.SQLBindParameter(
            stmt_handle, 0, getattr(C, input_output_type), 0, 0, 0, 0, C.NULL, 0, C.NULL
        )
        == C.SQL_ERROR
    )
...
@pytest.mark.parametrize(
    "value_type", ["SQL_C_WCHAR", "SQL_C_FLOAT", "SQL_C_SSHORT", "SQL_C_ULONG"]
)
