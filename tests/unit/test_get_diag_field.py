...
def test_invalid_handle(C):
    result = C.SQLGetDiagField(
        C.SQL_HANDLE_STMT, C.NULL, 0, 0, C.NULL, 0, C.NULL
    )
...
def test_mismatch_handle_type(C, env_handle):
    result = C.SQLGetDiagField(
        C.SQL_HANDLE_STMT, env_handle, 0, 0, C.NULL, 0, C.NULL
    )
    assert result == C.SQL_INVALID_HANDLE
...
def test_no_error(C, env_handle):
    result = C.SQLGetDiagField(
        C.SQL_HANDLE_ENV, env_handle, 0, 0, C.NULL, 0, C.NULL
    )
...
@pytest.mark.parametrize("diag_identifier, expected", [
    ("SQL_DIAG_NUMBER", 1),
    ("SQL_DIAG_NATIVE", 0),
])
...
@pytest.mark.parametrize("diag_identifier, buffer_length, expected_str, expected_len, expected_ret", [
...
def test_error_str(C, force_error, env_handle, diag_identifier, buffer_length, expected_str, expected_len, expected_ret):
