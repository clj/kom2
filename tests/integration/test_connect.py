...
def test_connect_without_server(driver_name):
    with pytest.raises(pypyodbc.DatabaseError) as exception:
        pypyodbc.connect(f"Driver={driver_name}")
...
def test_connect_no_credentials(driver_name):
    with pytest.raises(pypyodbc.DatabaseError) as exception:
        pypyodbc.connect(f"Driver={driver_name};server=asdf")
...
def test_connect_invalid_server(driver_name):
    with pytest.raises(pypyodbc.DatabaseError) as exception:
        pypyodbc.connect(f"Driver={driver_name};server=asdf://asdf;apitoken=asdf")
...
def test_connect_log(driver_name, tmp_path):
    ...
    with pytest.raises(pypyodbc.DatabaseError) as exception:
        pypyodbc.connect(f"Driver={driver_name};logfile={logfile}")
